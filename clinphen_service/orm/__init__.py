...
def init_db(uri=None):
    ...
    """
    global _ENGINE
    import clinphen_service.orm.models # noqa401 #pylint: disable=unused-variable
    if not uri:
...
def dump(obj, nonulls=False, fields=None):
    """
    Generate dictionary  of fields without SQLAlchemy internal fields
    & relationships
    """
    rels = ["ontology", "referenceset"]
    selected_fields = fields

    if not fields:
        selected_fields = [k for k in vars(obj).keys()
                           if not k.startswith('_') and k not in rels]

    if not nonulls:
        return {k: vars(obj)[k] for k in selected_fields}

    return {k: vars(obj)[k] for k in selected_fields and vars(obj)[k]}
